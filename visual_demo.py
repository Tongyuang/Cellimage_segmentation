...
def visual(img, gt):
    img = cv2.imread(img, -1) # do not modify the initial image
    gt = cv2.imread(gt, -1)
    label = np.unique(gt)
    height, width = img.shape[:2]
    visual_img = np.zeros((height, width, 3))# img for visualization
    for lab in label:
        if lab == 0:
            continue
        color = np.random.randint(low=0, high=255, size=3)# random color
...
if __name__ == "__main__":
    ...
    images = sorted([osp.join(image_path, img) for img in os.listdir(image_path) if img.find('.tif') != -1])[-8:]
    gts = sorted([osp.join(gt_path, gt) for gt in os.listdir(gt_path) if gt.find('.tif') != -1])[-8:]
    visual_path = './dataset2/visual'
    if not osp.exists(visual_path):
        os.mkdir(visual_path)
    count = 160
    for idx, (image, gt) in enumerate(zip(images, gts)):
